...
def SetOxideParameters(device, region):
    ...
    set_parameter(device=device, region=region, name="thermalconductivity",     value=Oxidethermalconductivity)
...
def SetSiliconParameters(device, region):
    ...
    set_parameter(device=device, region=region, name="thermalconductivity",     value=Sithermalconductivity)
...
def CreateJouleHeatSilicon(device,region):
    ...
    CreateEdgeModel(device, region, "JouleHeat", Heatsource)
    for v in ("Potential", "Electrons", "Holes", "Temperature"):
        CreateEdgeModelDerivatives(device, region, "JouleHeat", Heatsource, v)
...
def CreateTemperaturefield(device,region):
    ...
    equation(device=device, region=region, name="HeatconductionEquation", variable_name="Temperature",
            edge_volume_model="JouleHeat", edge_model="TemperatureEdgeFlux",
