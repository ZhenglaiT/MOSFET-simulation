...
def CreateElectronCurrent(device, region):
    ...
    #### test for requisite models here
#  Jn = "ElectronCharge*{0}*EdgeInverseLength*V_t_T*(Electrons@n1*Bern10 - Electrons@n0*Bern01)".format(mu_n)
    Jn = "ElectronCharge*mu_nT*EdgeInverseLength*edgeV_t_T*kahan3(Electrons@n1*Bern01,  Electrons@n1*vdiff,  -Electrons@n0*Bern01)"
#  Jn = "ElectronCharge*{0}*EdgeInverseLength*V_t_T*((Electrons@n1-Electrons@n0)*Bern01 +  Electrons@n1*vdiff)".format(mu_n)
...
def CreateHoleCurrent(device, region):
    ...
    if not InEdgeModelList(device, region, "Bern01"):
        CreateBernoulli(device, region)
    ##### test for requisite models here
#  Jp ="-ElectronCharge*{0}*EdgeInverseLength*V_t_T*(Holes@n1*Bern01 - Holes@n0*Bern10)".format(mu_p)
    Jp ="-ElectronCharge*mu_pT*EdgeInverseLength*edgeV_t_T*kahan3(Holes@n1*Bern01, -Holes@n0*Bern01, -Holes@n0*vdiff)"
#  Jp ="-ElectronCharge*{0}*EdgeInverseLength*V_t_T*((Holes@n1 - Holes@n0)*Bern01 - Holes@n0*vdiff)".format(mu_p)
